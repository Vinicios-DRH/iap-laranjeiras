...
class FormInscricaoTorneio(FlaskForm):
    nome_responsavel = StringField('Nome Completo do responsável', validators=[DataRequired()])
    email = StringField('E-mail do responsável', validators=[DataRequired(), Email()])
    ...
    aviso_informacoes = BooleanField('Desejo receber notificações no meu e-mail cadastrado.')
    lembrar_ciente = BooleanField('Estou ciente de pagar a primeira parcela sem direito a devolução')
